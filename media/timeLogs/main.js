...
(function () {
  ...
  /** т.к. состояние сохраняется то часто случается что записываешь время за прошедшую дату,
   * хотя интуитивно записываешь за сегодня - поэтому при каждом рендере скидываем дату на сегодня
   */
  setState({
    form: {
      for_date: getDateNow()
