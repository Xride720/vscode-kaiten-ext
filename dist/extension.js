...
exports.KaitenTaskViewProvider = void 0;
const vscode = __webpack_require__(1);
const string_1 = __webpack_require__(3);
class KaitenTaskViewProvider {
    ...
    _getHtmlForWebview(webview) {
        ...
        const nonce = (0, string_1.getNonce)();
        return `<!DOCTYPE html>
			<html lang="en">
			<head>
				<meta charset="UTF-8">
    ...
    _generateTaskInfoHtml(data) {
        const description = [];
        data.description?.split("\n\n").forEach(item => {
            if (item)
                description.push(`<p class="desc__list-item">${(0, string_1.convertKaitenDescription)(item)}</p>`);
        });
...
exports.KaitenTaskViewProvider = KaitenTaskViewProvider;
KaitenTaskViewProvider.viewType = 'kaiten.task';
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.formatTime = exports.formatDate = exports.getNonce = exports.convertKaitenDescription = exports.formatUrl = exports.generateKaitenLink = void 0;
function generateKaitenLink(base, branchName) {
    const result = /(\d{7,8})/.exec(branchName);
    if (!result)
        return null;
    return `${base}/${result[1]}`;
}
exports.generateKaitenLink = generateKaitenLink;
const formatUrl = (url) => {
    return url?.endsWith('/') ? url.slice(0, url.length - 1) : url;
};
exports.formatUrl = formatUrl;
const convertKaitenDescription = (desc) => {
    return desc.replaceAll(/\[([\W\w]*)\]\(([\W\w]*)\)/g, (a, b, c) => {
        return `<a href="${c}">${b || c}</a>`;
    });
};
exports.convertKaitenDescription = convertKaitenDescription;
function getNonce() {
    let text = '';
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.KaitenTaskStore = void 0;
const kaiten_1 = __webpack_require__(5);
const task_provider_1 = __webpack_require__(2);
const vscode = __webpack_require__(1);
const git_1 = __webpack_require__(40);
const string_1 = __webpack_require__(3);
const time_log_provider_1 = __webpack_require__(41);
const checklist_provider_1 = __webpack_require__(43);
const time_log_controller_1 = __webpack_require__(44);
const checklist_controller_1 = __webpack_require__(45);
class KaitenTaskStore {
    constructor(context, baseUrl, apiKey) {
        this._taskUrl = '';
        this._taskId = '';
        this.taskData = null;
        this._timeLogs = [];
        ...
        this.kaitenApi = new kaiten_1.KaitenApiService(baseUrl, apiKey);
        this.providerKaitenTask = new task_provider_1.KaitenTaskViewProvider(context.extensionUri);
        this.providerKaitenTimeLog = new time_log_provider_1.KaitenTimeLogViewProvider(context.extensionUri, this);
        this.providerKaitenCheckList = new checklist_provider_1.KaitenCheckListProvider(this);
        this.viewCheckList = vscode.window.createTreeView(checklist_provider_1.KaitenCheckListProvider.viewType, { treeDataProvider: this.providerKaitenCheckList });
        this.providerKaitenCheckList.view = this.viewCheckList;
        this.timeLogController = new time_log_controller_1.TimeLogController(context, this);
        this.checkListController = new checklist_controller_1.CheckListController(context, this);
    }
    async init() {
        const $this = this;
        [this.gitExt] = await Promise.all([(0, git_1.getBuiltInGitApi)(), this.initRoles()]);
        if (!this.gitExt)
            return;
        const initBranch = (_repo) => {
            const callback = () => {
                if (_repo.state.HEAD?.name) {
                    this.taskUrl = (0, string_1.generateKaitenLink)($this.baseUrl, _repo.state.HEAD.name) || '';
    ...
    async initRoles() {
        const result = await this.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.kaitenApi.getRoles());
        if (!result.error) {
    ...
    async updateTaskData() {
        const response = await this.withProgress(task_provider_1.KaitenTaskViewProvider.viewType, this.kaitenApi.getTask(this.taskId));
    ...
    async updateTimeLogs() {
        const response = await this.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.kaitenApi.getTimeLogs(this.taskId));
        if (!response.error && response.data) {
    ...
    async withProgress(viewId, promise) {
...
exports.KaitenTaskStore = KaitenTaskStore;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.KaitenApiService = void 0;
...
class KaitenApiService {
    ...
    async getTask(id) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + id, this.kaitenOptions({
                method: 'GET'
            }));
    ...
    async getRoles() {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/user-roles', this.kaitenOptions({
                method: 'GET'
            }));
            return { error: false, data: result };
        }
    ...
    async getTimeLogs(taskId) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/time-logs', this.kaitenOptions({
                method: 'GET'
            }));
            return { error: false, data: result };
        }
    ...
    async addTimeLog(taskId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/time-logs', this.kaitenOptions({
                method: 'POST',
    ...
    async updateTimeLog(taskId, logId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/time-logs/' + logId, this.kaitenOptions({
                method: 'PATCH',
                body: JSON.stringify(payload)
            }));
            return { error: false, data: result };
    ...
    async removeTimeLog(taskId, logId) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/time-logs/' + logId, this.kaitenOptions({
                method: 'DELETE'
            }));
    ...
    async addCheckList(taskId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/checklists', this.kaitenOptions({
                method: 'POST',
                body: JSON.stringify(payload)
            }));
            return { error: false, data: result };
    ...
    async updateCheckList(taskId, checklistId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/checklists/' + checklistId, this.kaitenOptions({
                method: 'PATCH',
                body: JSON.stringify(payload)
            }));
            return { error: false, data: result };
    ...
    async addCheckListItem(taskId, checklistId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/checklists/' + checklistId + '/items', this.kaitenOptions({
                method: 'POST',
                body: JSON.stringify(payload)
            }));
            return { error: false, data: result };
    ...
    async updateCheckListItem(taskId, checklistId, itemId, payload) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/checklists/' + checklistId + '/items/' + itemId, this.kaitenOptions({
                method: 'PATCH',
                body: JSON.stringify(payload)
            }));
            return { error: false, data: result };
    ...
    async deleteCheckListItem(taskId, checklistId, itemId) {
        try {
            const result = await this.request(this.baseUrl + '/api/latest/cards/' + taskId + '/checklists/' + checklistId + '/items/' + itemId, this.kaitenOptions({
                method: 'DELETE'
            }));
    ...
    request(url, options) {
        return new Promise((resolve, reject) => {
            try {
                (0, node_fetch_1.default)(url, options).then(async (res) => {
                    if (res.status === 401) {
...
exports.KaitenApiService = KaitenApiService;
...
/* harmony import */ var _response_js__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(37);
/* harmony import */ var _headers_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(26);
/* harmony import */ var _request_js__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(28);
/* harmony import */ var _errors_fetch_error_js__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(33);
/* harmony import */ var _errors_abort_error_js__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(39);
/* harmony import */ var _utils_is_redirect_js__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(38);
/* harmony import */ var formdata_polyfill_esm_min_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(13);
/* harmony import */ var _utils_is_js__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(31);
/* harmony import */ var _utils_referrer_js__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(34);
/* harmony import */ var fetch_blob_from_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(21);
/**
 * Index.js
 *
...
/* harmony export */ });
/* harmony import */ var fetch_blob__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(14);
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var _streams_cjs__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(15);
...
if (!globalThis.ReadableStream) {
  ...
  try {
    ...
    try {
      process.emitWarning = () => {}
      Object.assign(globalThis, __webpack_require__(17))
...
try {
  ...
  const { Blob } = __webpack_require__(19)
...
/* harmony export */ });
/* harmony import */ var node_fs__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(22);
/* harmony import */ var node_path__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(23);
/* harmony import */ var node_domexception__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(24);
/* harmony import */ var _file_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(20);
/* harmony import */ var _index_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(14);
...
if (!globalThis.DOMException) {
  try {
    const { MessageChannel } = __webpack_require__(25),
...
/* harmony export */ });
/* harmony import */ var node_util__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(27);
...
/* harmony export */ });
/* harmony import */ var node_url__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(29);
/* harmony import */ var node_util__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(27);
/* harmony import */ var _headers_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(26);
/* harmony import */ var _body_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(30);
/* harmony import */ var _utils_is_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(31);
/* harmony import */ var _utils_get_search_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(36);
/* harmony import */ var _utils_referrer_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(34);
/**
 * Request.js
 *
...
/* harmony import */ var fetch_blob__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(14);
/* harmony import */ var formdata_polyfill_esm_min_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(13);
/* harmony import */ var _errors_fetch_error_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(33);
/* harmony import */ var _errors_base_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(32);
/* harmony import */ var _utils_is_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(31);
...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await __webpack_require__.e(/* import() */ 1).then(__webpack_require__.bind(__webpack_require__, 46));
...
/* harmony export */ });
/* harmony import */ var node_net__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(35);
...
/* harmony export */ });
/* harmony import */ var _headers_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(26);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getBuiltInGitApi = void 0;
const vscode = __webpack_require__(1);
async function getBuiltInGitApi() {
    try {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.KaitenTimeLogViewProvider = void 0;
const vscode = __webpack_require__(1);
const string_1 = __webpack_require__(3);
const html_1 = __webpack_require__(42);
class KaitenTimeLogViewProvider {
    get view() {
        return this._view;
    }
    constructor(_extensionUri, store) {
        this._extensionUri = _extensionUri;
        this.store = store;
    }
    async resolveWebviewView(webviewView, context, _token) {
    ...
    updateTimeSummary() {
        if (this._view) {
            ...
            this._view.description = (0, string_1.formatTime)(summSpent);
        }
    }
    ...
    _getHtmlForWebview(webview) {
        ...
        const nonce = (0, string_1.getNonce)();
        const forDate = (0, html_1.Input)('for_date', 'Дата', '<input type="date" name="for_date" placeholder="Выберите дату" />');
        const timeSpent = (0, html_1.Input)('time_spent', 'Затрачено времени', '<input type="number" name="time_spent" placeholder="в минутах" />');
        const roleSelect = (0, html_1.Input)('role_id', 'Роль', `
				<select
					name="role_id"
					id="role_select"
			...
			`, 'select-container');
        const comment = (0, html_1.Input)('comment', 'Комментарий', '<textarea name="comment"></textarea>', 'cont-comment');
        return `<!DOCTYPE html>
			<html lang="en">
			<head>
				<meta charset="UTF-8">
    ...
    _generateTimeLogListHtml(data) {
        return [...data].sort((a, b) => new Date(a.for_date) > new Date(b.for_date) ? -1 : 1).map(item => `
			<li class="time-log__item" data-id="${item.id}">
				<div class="top-block">
					<span class="time-log__item-for_date" title="Дата: ${(0, string_1.formatDate)(item.for_date)}">${(0, string_1.formatDate)(item.for_date)}</span>
					<span class="time-log__item-role_name" title="Роль: ${item.role.name}">${item.role.name}</span>
					<span
						class="time-log__item-time_spent"
						title="Затрачено времени: ${(0, string_1.formatTime)(item.time_spent)}"
					>
						${(0, string_1.formatTime)(item.time_spent)}
					</span>
				</div>
    ...
    _generateRoleOptionsHtml(data) {
        return data.map(item => `
			<option label="${item.name}">${item.id}</option>
		`).join("\n");
    }
}
exports.KaitenTimeLogViewProvider = KaitenTimeLogViewProvider;
KaitenTimeLogViewProvider.viewType = 'kaiten.time-log';
...
const Input = (name, label, inputHtml, className) => `
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CheckListItem = exports.CheckList = exports.KaitenCheckListProvider = void 0;
const vscode = __webpack_require__(1);
class KaitenCheckListProvider {
    constructor(store) {
    ...
    set view(_view) {
        ...
        if (this._view) {
            this._view.onDidChangeCheckboxState(async (e) => {
                if (!e.items || !e.items.length || !e.items[0][0])
                    return;
                const item = e.items[0][0];
                if (item instanceof CheckList)
                    return;
                await this.store.checkListController.changeCheckListItem(item.data.id, item.data.checklist_id, { checked: item.checkboxState === vscode.TreeItemCheckboxState.Checked });
            });
        }
    }
    get view() {
    ...
    getChildren(element) {
        if (element && element instanceof CheckListItem)
    ...
    getNodeList(checkListId) {
        const checklists = (this.store.taskData?.checklists || []).slice();
        if (!checkListId) {
            checklists.sort((a, b) => a.sort_order > b.sort_order ? 1 : -1);
            return checklists.map(item => new CheckList(item, vscode.TreeItemCollapsibleState.Collapsed));
        }
        else {
            const checklist = checklists.find(item => item.id === checkListId);
            if (!checklist)
                return [];
            return checklist.items?.slice().sort((a, b) => a.sort_order > b.sort_order ? 1 : -1).map(item => new CheckListItem(item)) || [];
        }
    }
}
exports.KaitenCheckListProvider = KaitenCheckListProvider;
KaitenCheckListProvider.viewType = 'kaiten.checklist';
class CheckList extends vscode.TreeItem {
    constructor(data, collapsibleState, command) {
        super(data.name, collapsibleState);
        this.data = data;
        this.collapsibleState = collapsibleState;
        this.command = command;
        this.contextValue = 'kaiten.checklist.checklist';
...
class CheckListItem extends vscode.TreeItem {
    constructor(data, command) {
        ...
        this.data = data;
        this.command = command;
        this.contextValue = 'kaiten.checklist.checklistItem';
...
class TimeLogController {
    constructor(context, store) {
        this.store = store;
        context.subscriptions.push(vscode.commands.registerCommand('kaiten.time-log.refresh', async () => {
            store.providerKaitenTimeLog.clearForm();
            await store.updateTimeLogs();
        }));
    }
    async updateTimeLogs() {
        const response = await this.store.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.store.kaitenApi.getTimeLogs(this.store.taskId));
        if (!response.error && response.data) {
            this.store.timeLogs = response.data;
        }
    }
    async addTimeLog(payload) {
        const response = await this.store.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.store.kaitenApi.addTimeLog(this.store.taskId, payload));
        if (!response.error) {
    ...
    async updateTimeLog(payload) {
        const response = await this.store.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.store.kaitenApi.updateTimeLog(this.store.taskId, payload.id, payload));
        if (!response.error) {
            await this.updateTimeLogs();
        }
        else {
    ...
    async removeTimeLog(logId) {
        ...
        if (confirm !== 'Да')
            return;
        const response = await this.store.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.store.kaitenApi.removeTimeLog(this.store.taskId, logId));
        if (!response.error) {
            await this.updateTimeLogs();
        }
        else {
...
class CheckListController {
    constructor(context, store) {
        this.store = store;
        context.subscriptions.push(vscode.commands.registerCommand('kaiten.checklist.add', async () => {
            await this.addCheckList();
        }), vscode.commands.registerCommand('kaiten.checklist.edit', async (list) => {
            await this.editCheckList(list);
        }), vscode.commands.registerCommand('kaiten.checklist.delete', async (list) => {
            await this.deleteCheckList(list);
        }), vscode.commands.registerCommand('kaiten.checklist.add-item', async (list) => {
            await this.addCheckListItem(list);
        }), vscode.commands.registerCommand('kaiten.checklist.edit-item', async (item) => {
            await this.editCheckListItem(item);
        }), vscode.commands.registerCommand('kaiten.checklist.delete-item', async (item) => {
    ...
    async addCheckList() {
        ...
        if (!input)
            return;
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.addCheckList(this.store.taskId, { name: input }));
        if (!response.error) {
    ...
    async editCheckList(list) {
        ...
        if (!input)
            return;
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.updateCheckList(this.store.taskId, list.data.id, {
            name: input
        }));
        if (!response.error) {
            if (this.store.taskData) {
    ...
    async deleteCheckList(list) {
        ...
        if (confirm !== 'Да')
            return;
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.deleteCheckList(this.store.taskId, id));
        if (!response.error) {
            if (this.store.taskData) {
    ...
    async addCheckListItem(checklist) {
        ...
        if (!input)
            return;
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.addCheckListItem(this.store.taskId, checklistId, { text: input }));
        if (!response.error) {
            if (this.store.taskData) {
    ...
    async changeCheckListItem(id, checklistId, payload) {
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.updateCheckListItem(this.store.taskId, checklistId, id, payload));
        if (!response.error) {
            if (this.store.taskData) {
                this.store.taskData.checklists = this.store.taskData.checklists.map(list => list.id === checklistId ? {
                    ...list,
    ...
    async deleteCheckListItem(item) {
        ...
        if (confirm !== 'Да')
            return;
        const response = await this.store.withProgress(checklist_provider_1.KaitenCheckListProvider.viewType, this.store.kaitenApi.deleteCheckListItem(this.store.taskId, checklistId, id));
        if (!response.error) {
            if (this.store.taskData) {
                this.store.taskData.checklists = this.store.taskData.checklists.map(list => list.id === checklistId ? {
                    ...list,
...
async function activate(context) {
    ...
    const kaitenUrl = (0, string_1.formatUrl)(config.get('url')) || '';
    const kaitenApiKey = config.get('apiKey') || '';
    if (!kaitenUrl || !kaitenApiKey) {
        vscode.window.showErrorMessage(`Необходимо заполнить настройки расширения Kaiten (Api Key + Url)`);
        return;
    }
    const store = new store_1.KaitenTaskStore(context, kaitenUrl, kaitenApiKey);
    await store.init();
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(task_provider_1.KaitenTaskViewProvider.viewType, store.providerKaitenTask), vscode.window.registerWebviewViewProvider(time_log_provider_1.KaitenTimeLogViewProvider.viewType, store.providerKaitenTimeLog), store.viewCheckList);
    context.subscriptions.push(vscode.commands.registerCommand('kaiten.task.open', () => {
