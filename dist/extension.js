...
class KaitenTaskStore {
        ...
        if (this.gitExt.state === 'initialized' && this.gitExt.repositories[0]) {
            ...
            repo.state.onDidChange(() => {
                if (repo.state.HEAD?.name) {
                    this.taskUrl = (0, string_1.generateKaitenLink)($this.baseUrl, repo.state.HEAD.name) || '';
    ...
    async addTimeLog(payload) {
        const response = await this.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.kaitenApi.addTimeLog(this.taskId, payload));
        if (!response.error) {
    ...
    async updateTimeLog(payload) {
        const response = await this.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.kaitenApi.updateTimeLog(this.taskId, payload.id, payload));
        if (!response.error) {
            await this.updateTimeLogs();
        }
        else {
    ...
    async removeTimeLog(logId) {
        ...
        if (confirm !== 'Да')
            return;
        const response = await this.withProgress(time_log_provider_1.KaitenTimeLogViewProvider.viewType, this.kaitenApi.removeTimeLog(this.taskId, logId));
        if (!response.error) {
            await this.updateTimeLogs();
        }
        else {
...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await __webpack_require__.e(/* import() */ 1).then(__webpack_require__.bind(__webpack_require__, 43));
...
async function activate(context) {
    ...
    await store.init();
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(task_provider_1.KaitenTaskViewProvider.viewType, store.providerKaitenTask), vscode.window.registerWebviewViewProvider(time_log_provider_1.KaitenTimeLogViewProvider.viewType, store.providerKaitenTimeLog));
